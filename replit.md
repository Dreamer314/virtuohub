...
-   **Production-Grade Likes System**: Toggle-based likes for posts and comments with one-like-per-user enforcement. Uses junction tables (`post_likes`, `comment_likes`) with unique constraints to prevent duplicate likes. Backend provides toggle semantics (like/unlike), frontend displays optimistic updates with visual feedback (filled icons). API returns `{ likes: number, hasLiked: boolean }` for state synchronization.
-   **Creator Profile System**: Stores professional details (roles, platforms, tools, experience, portfolio, social links) in the `quick_facts` JSONB column, enabling creators to showcase their skills. Features a "Find Talent" directory for searching public creator profiles with comprehensive filtering. Uses virtual world/immersive creator terminology (e.g., "Avatar clothing creator", "World or sim builder", "Rigger (bodies or clothing)") reflecting the platform's specialized audience.
-   **Centralized Creator Options**: All creator profile options (PRIMARY_ROLES, PLATFORMS, TOOLS, EXPERIENCE_LEVELS, JOB_TYPES, PAYMENT_TYPES, CURRENCIES) are defined in `client/src/config/creatorOptions.ts` and imported across profile settings, talent directory filters, and job posting forms to ensure consistency. Profile settings now use "Primary role" label with helper text "What you are best known for in virtual worlds" instead of generic "Primary Skill".
