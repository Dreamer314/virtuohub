...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // VHub Pulse Routes
  app.get("/api/pulse/polls", async (req, res) => {
    try {
      const { status } = req.query;
      const validStatus = status === 'active' || status === 'completed' ? status : undefined;
      const polls = await storage.getPulsePolls(validStatus);
      res.json(polls);
    } catch (error) {
      res.status(500).json({ message: "Failed to fetch pulse polls" });
    }
  });

  app.get("/api/pulse/polls/:id", async (req, res) => {
    try {
      const { id } = req.params;
      const poll = await storage.getPulsePoll(id);
  ...
  app.post("/api/pulse/polls/:id/vote", async (req, res) => {
    try {
      const { id } = req.params;
      const { optionIndex } = req.body;
      ...
      const updatedPoll = await storage.voteOnPulsePoll(id, optionIndex);
      
      if (!updatedPoll) {
        return res.status(404).json({ message: "Poll not found or voting not allowed" });
      }
  ...
  app.post("/api/pulse/polls/:id/engage", async (req, res) => {
    try {
      const { id } = req.params;
      ...
      const updatedPoll = await storage.updatePulsePollEngagement(id, action);
  ...
  app.get("/api/pulse/reports", async (req, res) => {
    try {
      ...
      const validAccessType = ['free', 'paid', 'private'].includes(accessType as string) ? accessType as 'free' | 'paid' | 'private' : undefined;
      const reports = await storage.getPulseReports(validAccessType);
      res.json(reports);
    } catch (error) {
      res.status(500).json({ message: "Failed to fetch pulse reports" });
    }
  });

  app.get("/api/pulse/reports/:id", async (req, res) => {
    try {
      const { id } = req.params;
      const report = await storage.getPulseReport(id);
