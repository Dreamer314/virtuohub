...
// Component to render a single comment with profiles_v2 data
function CommentItem({ comment }: { comment: SpotlightCommentRow }) {
  // Fetch profile from profiles_v2 using user_id
  const { data: profile } = useUserProfile(comment.user_id || '');

  // Get display data using utility functions that support both formats
  const displayName = getDisplayName(profile, 'User');
  const avatarUrl = getAvatarUrl(profile);
  const initial = displayName.charAt(0).toUpperCase();

  return (
    <div className="flex space-x-3" data-testid={`comment-${comment.id}`}>
      {avatarUrl ? (
        <img 
          src={avatarUrl} 
          alt={displayName}
...
export function SpotlightEngagement({ spotlightId }: SpotlightEngagementProps) {
  ...
  // Fetch comments (user_id only, we'll resolve profiles_v2 per comment)
